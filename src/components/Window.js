...
class Window extends React.Component {
  ...
  renderInfoWindowIcons() {
    ...
    if (infoWindow.experiences && infoWindow.wishlist)
      return (
        <div>
          <GiFire />
          <MdStar />
    ...
    if (infoWindow.experiences)
      return (
        <div>
          <GiFire style={{ color: colors.experiences }} />
        </div>
      );
    if (infoWindow.wishlist)
      return (
        <div>
          <MdStar style={{ color: colors.wishlist }} />
        </div>
      );
  }
  ...
  renderInfoWindowDate() {
    ...
    return (
      <div className="infoWindow-date-container">
        ...
        <div
          className="infoWindow-icon-trash plain-link"
          onClick={() => removeMarker(user.id, infoWindow)}
        >
  ...
  render() {
    const { infoWindow, markers, closeInfoWindow, home, setHome, user } =
      this.props;
    return (
      <InfoWindow
      ...
      >
        <div className="infoWindow">
            ...
            markers.indexOf(infoWindow) === -1 && (
              <>
                ...
                <div className="infoWindow-button-container">
                  <div
                    className="infoWindow-button experiences"
                    onClick={() => this.submitPlace("experiences")}
                  >
                    Add Experience
                  </div>
                  <div
                    className="infoWindow-button wishlist"
                    onClick={() => this.submitPlace("wishlist")}
                  >
                    Add Wish
                  </div>
                </div>
          ...
          {!home &&
            ...
            this.renderInfoWindowDate()}

          {/* Add Home */}
          {home && infoWindow !== user.home && (
            <div
              className="infoWindow-button user-home set-home-active"
              onClick={() => setHome(user.id, infoWindow)}
            >
              Set Home
            </div>
          )}
          ...
          {/* Existing Home */}
          {infoWindow === user.home && (
            <div className="infoWindow-button user-home">Home</div>
          )}
        </div>
      </InfoWindow>
    );
...
const mapDispatch = (dispatch) => ({
  addMarker: (id, marker, date, category) =>
    dispatch(addMarker(id, marker, date, category)),
